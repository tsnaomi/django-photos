...
class TestModels(TestCase):
    ...
    # def test_tag(self):
    #     tag = Tag.objects.create(creator=self.user, name='Ravenclaw')
    #     photo = self.generate_photo(self.filename1, self.user)
    #     photo.tags.add(tag)
    #     self.assertEqual(tag.name, 'Ravenclaw')
    #     self.assertEqual(unicode(tag), u'Ravenclaw')
    #     self.assertIn(tag, photo.tags.all())

    # def test_tag_NAY_USER(self):
    #     with self.assertRaises(IntegrityError):
    #         Tag.objects.create(name='Ravenclaw')

    # def test_tag_NAY_NAME(self):
    #     tag = Tag(creator=self.user)
